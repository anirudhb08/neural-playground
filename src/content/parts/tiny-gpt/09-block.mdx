...
The thinking piece is built from one small idea. Take a row of numbers `w`
that belongs to the model — weights, adjusted by training like every matrix
so far, not anything computed from the input. Dot it against a position's
gathered vector and clip the result at zero:
...
pattern, in proportion to the match, and stays silent otherwise. The clip is
the **bend**: plot `fires` against `w · gathered` and you get a straight line
bent flat at zero — a poor match does not fire a little, it does not fire at
all. (The standard name for the bend is **ReLU**.) With `w = [1, −1]`:
...
Firing alone changes nothing — a unit must *do* something when it fires. So
each unit also owns a second row of weights, its **contribution**: what gets
added into the position's description, in proportion to how hard the unit
fired. The whole unit reads: *if the gathered evidence looks like my pattern,
add my contribution.*

The **feed-forward** layer is 128 such units — four times the model's width,
by convention — all reading the same gathered vector. Their patterns, stacked
as the columns of one matrix, are `W1`; their contributions, stacked as the
rows of another, are `W2`. One line asks every if and adds up what fired:
